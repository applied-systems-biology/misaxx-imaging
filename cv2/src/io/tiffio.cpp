...
cv::Mat cv::tiffread(const boost::filesystem::path &t_path) {
    ...
    for(int row = 0; row < static_cast<int>(reader.get_image_height()); ++row) {
        reader.read_row_(result.ptr(row), row);
    }
    return result;
}

void cv::tiffwrite(const cv::Mat &t_img, const boost::filesystem::path &t_path) {
